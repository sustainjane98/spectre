...
const REPO = "sustainjane/spectre";
const REPO_READONLY = "sustainjane/spectre";
...
async function generateCriticalCSS(done, category, url) {
    try {
        ...
        const externalCssResponse = await axios.get(
            "https://do.nrw.linksjugend-solid.de/public/cards.min.css"
        );
        ...
        const criticalCss = await penthouse({
            ...
            forceInclude: [
                ...
                "/.kg-blockquote-alt/g",
                "/.kg-header-card/g",
                "/.kg-header-card-content/g",
                "/.kg-header-card-image/g",
            ],
            renderWaitTime: 1000,
        ...
        const regex = /url\((?:'|"|)(\.\.\/)([^)'"]+)(?:'|"|)\)/g;
        const cleanedCss = criticalCss.replace(
            regex,
            (match, p1, p2) => `url('/assets/${p2}')`
        );
        fs.writeFileSync(
            `partials/css/${category}.critical.css.hbs`,
            `<style>\n${cleanedCss}\n</style>`
        );
    } catch (err) {
        console.error(err);
...
const cssWatcher = () =>
    watch(["assets/css/screen.css", "assets/css/print.css"], css);
